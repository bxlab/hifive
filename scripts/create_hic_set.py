...
def main():
    ...
    if len(sys.argv) < 8:
        ...
        print "MIN_DIST          The smallest interaction distance to be included for filtering fends."
        ...
        return None
    data_fname, hic_fname, mininteractions, mindistance, maxdistance, minsize, numbins = sys.argv[1:8]
    mininteractions, mindistance, maxdistance, minsize, numbins = (
        int(mininteractions), int(mindistance), int(maxdistance), int(minsize), int(numbins))
    if rank == 0:
        hic = hifive.HiC(hic_fname, 'w')
        hic.load_data(data_fname)
        hic.filter_fends(mininteractions=mininteractions, mindistance=mindistance, maxdistance=maxdistance)
        hic.save()
    ...
    hic.find_distance_means(minsize=minsize, numbins=numbins)
    if rank == 0:
        hic.save()
    return None
