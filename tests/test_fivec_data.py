...
class FiveCData(unittest.TestCase):
    def setUp(self):
        self.data = h5py.File('tests/data/test_import.fcd', 'r')
        self.frag_fname = 'tests/data/test.frags'
        self.count_fname = 'tests/data/test.counts'
        self.bam_fname1 = 'tests/data/test_fivec_1.bam'
        self.bam_fname2 = 'tests/data/test_fivec_2.bam'

    def test_fivec_counts_data_creation(self):
        subprocess.call("./bin/hifive 5c-data -q -C %s %s tests/data/test_temp.fcd" %
                        (self.count_fname, self.frag_fname), shell=True)
        data = h5py.File('tests/data/test_temp.fcd', 'r')
        self.compare_hdf5_dicts(self.data, data, 'data')

    def test_fivec_bam_data_creation(self):
        if 'pysam' not in sys.modules.keys():
            print("pysam required for bam import", file=sys.stderr)
            return None
        subprocess.call("./bin/hifive 5c-data -q -B %s %s %s tests/data/test_temp.fcd" %
                        (self.bam_fname1, self.bam_fname2, self.frag_fname), shell=True)
        data = h5py.File('tests/data/test_temp.fcd', 'r')
        self.compare_hdf5_dicts(self.data, data, 'data')

    def tearDown(self):
        subprocess.call('rm -f tests/data/test_temp.fcd', shell=True)
