...
class FiveCProject(unittest.TestCase):
    def setUp(self):
        self.data_fname = 'tests/data/test.fcd'
        self.project_fname = 'tests/data/test.fcp'
        self.probability_fname = 'tests/data/test_probability.fcp'
        self.express_fname = 'tests/data/test_express.fcp'
        self.binning_fname = 'tests/data/test_binning.fcp'
    ...
    def test_fivec_project_preanalysis(self):
        subprocess.call("./bin/hifive 5c-project -q -f 20 %s tests/data/test_temp.fcp" %
                        self.data_fname, shell=True)
        project = h5py.File('tests/data/test_temp.fcp', 'r')
        self.compare_hdf5_dicts(self.raw, project, 'project')

    def test_fivec_project_probability(self):
        subprocess.call("./bin/hifive 5c-normalize probability -q -o tests/data/test_temp.fcp -b 100 -l 0.5 -p %s" %
                        self.project_fname, shell=True)
        project = fivec.FiveC("tests/data/test_temp.fcp", 'r', silent=True)
    ...
    def test_fivec_project_express(self):
        subprocess.call("./bin/hifive 5c-normalize express -q -o tests/data/test_temp.fcp -e 100 -d -w cis %s" %
                        self.project_fname, shell=True)
        project = fivec.FiveC("tests/data/test_temp.fcp", 'r', silent=True)
    ...
    def test_fivec_project_binning(self):
        subprocess.call("./bin/hifive 5c-normalize binning -q -o tests/data/test_temp.fcp -i 10 -t 1.0 -y cis -v len -n 5 -u even %s" %
                        self.project_fname, shell=True)
        project = fivec.FiveC("tests/data/test_temp.fcp", 'r', silent=True)
    ...
    def tearDown(self):
        subprocess.call('rm -f tests/data/test_temp.fcp', shell=True)
