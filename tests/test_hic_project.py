...
class HiCProject(unittest.TestCase):
    def setUp(self):
        self.data_fname = 'tests/data/test.hcd'
        self.bin_data_fname = 'tests/data/test_bin.hcd'
        self.project_fname = 'tests/data/test.hcp'
        self.bin_project_fname = 'tests/data/test_bin.hcp'
        self.probbin_fname = 'tests/data/test_probbin.hcp'
        self.probpois_fname = 'tests/data/test_probpois.hcp'
        self.express_fname = 'tests/data/test_express.hcp'
        self.bin_express_fname = 'tests/data/test_bin_express.hcp'
        self.binning_fname = 'tests/data/test_binning.hcp'
    ...
    def test_hic_project_preanalysis(self):
        subprocess.call("./bin/hifive hic-project -q -m 20000 -f 10 -j 30000 -n 5 %s tests/data/test_temp.hcp" %
                        (self.data_fname), shell=True)
        project = h5py.File('tests/data/test_temp.hcp', 'r')
    ...
    def test_hic_project_probability_binomial(self):
        subprocess.call("./bin/hifive hic-normalize probability -q -m 20000 -o tests/data/test_temp.hcp -b 15 -l 0.4 -g 0.0015 -p %s" %
                        (self.project_fname), shell=True)
        project = hic.HiC("tests/data/test_temp.hcp", 'r', silent=True)
    ...
    def test_hic_project_probability_poisson(self):
        subprocess.call("./bin/hifive hic-normalize probability -q -m 20000 -o tests/data/test_temp.hcp -b 15 -l 0.4 -g 0.0015 -p -a poisson %s" %
                        (self.project_fname), shell=True)
        project = hic.HiC("tests/data/test_temp.hcp", 'r', silent=True)
    ...
    def test_hic_project_express(self):
        subprocess.call("./bin/hifive hic-normalize express -q -m 20000 -o tests/data/test_temp.hcp -e 100 -w cis -f 10 %s" %
                        (self.project_fname), shell=True)
        project = hic.HiC("tests/data/test_temp.hcp", 'r', silent=True)
    ...
    def test_hic_project_binning(self):
        subprocess.call("./bin/hifive hic-normalize binning -q -m 20000 -o tests/data/test_temp.hcp -r 5 -y cis -t 1.0 -v len,distance -s 3,3 -u even,fixed-const %s" %
                        (self.project_fname), shell=True)
        project = hic.HiC("tests/data/test_temp.hcp", 'r', silent=True)
    ...
    def tearDown(self):
        subprocess.call('rm -f tests/data/test_temp.hcp', shell=True)
