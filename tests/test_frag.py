...
class Frags(unittest.TestCase):
    def setUp(self):
        self.frags = h5py.File('tests/data/test.frags', 'r')
        self.bed_fname = 'tests/data/test_frag.bed'

    def test_fragment_creation(self):
        subprocess.call("./bin/hifive fragments -q -g mm9 -r HindIII %s tests/data/test_temp.frags" %
                        self.bed_fname, shell=True)
        frags = h5py.File('tests/data/test_temp.frags', 'r')
        self.compare_hdf5_dicts(self.frags, frags, 'frags')

    def tearDown(self):
        subprocess.call('rm -f tests/data/test_temp.frags', shell=True)
