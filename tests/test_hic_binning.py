...
class HiCBinning(unittest.TestCase):
    def setUp(self):
        self.project_fname = 'tests/data/test_probbin.hcp'
        self.binned_fname = 'tests/data/test_binned.hdf5'
        self.heatmap_fname = 'tests/data/test.hch'
    ...
    def test_generate_heatmap(self):
        subprocess.call("./bin/hifive hic-heatmap -q -b 50000 -t -F hdf5 -d fend %s tests/data/test_temp.hch" %
                        self.project_fname, shell=True)
        heatmap = h5py.File("tests/data/test_temp.hch")
        self.compare_hdf5_dicts(self.heatmap, heatmap, 'heatmap')
        
    def tearDown(self):
        subprocess.call('rm -f tests/data/test_temp.hch', shell=True)
