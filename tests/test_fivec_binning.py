...
class FiveCBinning(unittest.TestCase):
    def setUp(self):
        self.project_fname = 'tests/data/test_probability.fcp'
        self.binned_fname = 'tests/data/test_binned.hdf5'
        self.heatmap_fname = 'tests/data/test.fch'
    ...
    def test_generate_heatmap(self):
        subprocess.call("./bin/hifive 5c-heatmap -q -b 50000 -t -d fragment -a full %s tests/data/test_temp.fch" %
                        self.project_fname, shell=True)
        heatmap = h5py.File("tests/data/test_temp.fch")
        self.compare_hdf5_dicts(self.heatmap, heatmap, 'heatmap')

    def tearDown(self):
        subprocess.call('rm -f tests/data/test_temp.fch', shell=True)
